...
box server start name=ssd.innomega.se port=8080 host=127.0.1.1 cfengine=$CF_ENGINE serverConfigFile=$WEB_ROOT/ssd.innomega.se/server.json webroot=$WEB_ROOT/airsystems.se/www rewritesEnable=$REWRITES_ENABLED openbrowser=false saveSettings=true --force;
